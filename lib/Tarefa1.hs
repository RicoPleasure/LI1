...
{-|
  ...
  * 'Base': recorrendo á função TODO:. 
  * 'Torre': recorrendo á função TODO:.
  * 'Mapa': recorrendo á função TODO:.

  Para além da validação de cada tipo de objetos presente no 'Jogo', a função 'validaJogo' também valida se existem sobreposições entre os vários objetos, recorrendo á função 'verificaSobreposicoes'
...
validaJogo j@(Jogo { 
              ...
              baseJogo = base,
              portaisJogo = portais }) =
                validaPortais portais mapa base && not (verificaSobreposicoes j)
...
verificaSobreposicoes (Jogo {baseJogo = base, portaisJogo = portais, torresJogo = torres}) = 
    verificaCollisionPortais portais torres base
